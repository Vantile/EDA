...
class consultorio {
	HashMap< medico,colaPacientes > tabla;
public:
...
list<medico> consultorio::listadoMedicos() {
	list<medico> l;
	HashMap< medico,colaPacientes >::ConstIterator it = tabla.cbegin();
	HashMap< medico,colaPacientes >::ConstIterator itFin = tabla.cend();
	while (it != itFin) {
		l.push_back(it.key());
		++it;
	}
