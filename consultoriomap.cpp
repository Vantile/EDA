...
// Se utiliza el tipo de datos map de la libreria STL
...
class consultorio {
	map< medico,colaPacientes > tabla;
public:
...
// Añade un nuevo medico al consultorio
...
// lista de pacientes
void consultorio::nuevoMedico(medico const& m) {
	pair<map< medico,colaPacientes >::iterator, bool> par = tabla.insert(parejaCV(m,colaPacientes()));
	if (!par.second) { // el elemento ya existia
		(*par.first).second = colaPacientes();
	}
}

// Listado de los medicos en orden alfabetico
...
// Devuelve la cola de pacientes de un medico
